...
// `magic/skills/magic-framework/references/plugin-deeplink.md`, which is
// versioned by `magic`'s releases rather than by this package's, and therefore
// drifts silently. Measured once: the reference was stamped v0.0.2 against a
// shipped 0.1.0 and handed out a `handle(Uri)` signature that no longer exists,
// a `Route.to(...)` call that does not compile, and no platform setup at all.
//
...
const String _referencePath =
    '../magic/skills/magic-framework/references/plugin-deeplink.md';
...
void main() {
  test('the magic skill reference is stamped with the shipped version', () {
    ...
    expect(
      firstLine,
      contains('magic_deeplink v$version'),
