...
You are preparing a new release for the **Magic Deeplink** Flutter plugin. Follow this checklist precisely:

### Phase 1: Validation

1. **Branch check** — Must be on `master` branch. If not, STOP and warn.
...
3. **Categorize** — Use these emoji categories:
   - `### ✨ New Features` — new drivers, handlers, CLI commands
   - `### 🐛 Bug Fixes` — bug fixes
   - `### 🔧 Improvements` — DX, CI/CD, docs, refactors, performance
   - `### 📚 Documentation` — README, doc/ folder, CHANGELOG
   - `### ⚠️ Breaking Changes` — only if driver/handler contracts, config structure, or provider API changed
...
**pub.dev:** ✅ Validate passed → Published (run #{publish_run_id}) — https://pub.dev/packages/magic_deeplink
```
